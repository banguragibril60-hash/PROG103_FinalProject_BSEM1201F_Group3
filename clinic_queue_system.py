...
# -----------------------------
# Login System with Password Toggle
# -----------------------------
logged_in = False
...
served_label = tk.Label(dashboard_frame, image=served_icon, text="Today Served: 0", compound="top", bg="#E3F2FD",
                        fg="#0D47A1", font=("Arial", 12, "bold"))
served_label.grid(row=0, column=0, padx=20, pady=20)

balance_label = tk.Label(dashboard_frame, image=balance_icon, text="Queue Balance: 0", compound="top", bg="#E3F2FD",
                         fg="#0D47A1", font=("Arial", 12, "bold"))
balance_label.grid(row=0, column=1, padx=20, pady=20)

time_label = tk.Label(dashboard_frame, image=time_icon, text="Avg Service Time: --", compound="top", bg="#E3F2FD",
                      fg="#0D47A1", font=("Arial", 12, "bold"))
time_label.grid(row=0, column=2, padx=20, pady=20)

current_label = tk.Label(dashboard_frame, image=current_icon, text="Current Served: None", compound="top", bg="#E3F2FD",
                         fg="#0D47A1", font=("Arial", 12, "bold"))
current_label.grid(row=0, column=3, padx=20, pady=20)

# Scrollable Form Container Layout
...
# UI Form Matrix Fields
row = 0
make_label(reg_scrollable_frame, "Full Name", row)
name_entry = tk.Entry(reg_scrollable_frame, width=40);
name_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Date of Birth (YYYY-MM-DD)", row)
dob_entry = tk.Entry(reg_scrollable_frame, width=40);
dob_entry.grid(row=row, column=1, pady=5);
row += 1
...
gender_combo.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Phone Number", row)
phone_entry = tk.Entry(reg_scrollable_frame, width=40);
phone_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Email Address", row)
email_entry = tk.Entry(reg_scrollable_frame, width=40);
email_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Address", row)
address_entry = tk.Entry(reg_scrollable_frame, width=40);
address_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Ticket Number", row)
ticket_entry = tk.Entry(reg_scrollable_frame, width=40);
ticket_entry.grid(row=row, column=1, pady=5);
row += 1
...
checkin_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Appointment Time (HH:MM)", row)
appt_entry = tk.Entry(reg_scrollable_frame, width=40);
appt_entry.grid(row=row, column=1, pady=5);
row += 1
...
appt_type.grid(row=row, column=1, pady=5);
row += 1
...
provider_entry = tk.Entry(reg_scrollable_frame, width=40);
provider_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Blood Type", row)
blood_type = ttk.Combobox(reg_scrollable_frame, values=["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-", "Unknown"],
                          width=37)
blood_type.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Allergies", row)
allergies_entry = tk.Entry(reg_scrollable_frame, width=40);
allergies_entry.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Emergency Contact Name", row)
emergency_name = tk.Entry(reg_scrollable_frame, width=40);
emergency_name.grid(row=row, column=1, pady=5);
row += 1

make_label(reg_scrollable_frame, "Emergency Contact Phone", row)
emergency_phone = tk.Entry(reg_scrollable_frame, width=40);
emergency_phone.grid(row=row, column=1, pady=5);
row += 1
...
status_entry.grid(row=row, column=1, pady=5);
row += 1
...
priority_entry.grid(row=row, column=1, pady=5);
row += 1
...
# -----------------------------
def load_data_bridge(silent=False):
    # FIXED: Calling backend.load_data() cleanly without passing 'silent' into it
    success, msg = backend.load_data()
    if success is True and not silent:
        messagebox.showinfo("Load Records", msg)
    elif success is False:
        messagebox.showwarning("Load Error", msg)
    elif success is None and not silent:
        messagebox.showinfo("Load Records", msg)
...
def update_display():
    queue_text = "Queue:\n" + "\n".join([f"{i + 1}. {p['Full Name']} ({p['Ticket']}) - {p.get('Status', 'Waiting')}"
                                         for i, p in enumerate(backend.queue)]) if backend.queue else "Queue: Empty"
    processed_text = "Processed:\n" + "\n".join([f"{p['Full Name']} ({p['Ticket']})"
                                                 for p in backend.processed_patients[
                                                          -5:]]) if backend.processed_patients else "Processed: None"
    display.config(text=queue_text)
...
def process_patient():
    ...
    backend.save_data()
    update_display()
    messagebox.showinfo("Process Next", f"Processed patient: {backend.current_patient}")
...
def exit_app():
    backend.save_data()
    if messagebox.askyesno("Exit", "Save and exit application?"):
        root.destroy()
...
def clear_form():
    ...
    messagebox.showinfo("Clear Form", "Form cleared for new patient entry.")
...
tk.Button(reg_scrollable_frame, image=process_icon, text="Process Next", compound="left", command=process_patient,
          bg="#FF9800", fg="white", font=("Arial", 10, "bold")).grid(row=btn_row, column=1, padx=10, pady=8)
...
tk.Button(reg_scrollable_frame, text="📂 Load Records", command=lambda: load_data_bridge(False),
          bg="#FF9800", fg="white", font=("Arial", 10, "bold")).grid(row=btn_row + 2, column=0, padx=10, pady=8)

tk.Button(reg_scrollable_frame, image=clear_icon, text="Clear Queue", compound="left", command=clear_queue,
          bg="#F44336", fg="white", font=("Arial", 10, "bold")).grid(row=btn_row + 2, column=1, padx=10, pady=8)

tk.Button(reg_scrollable_frame, image=exit_icon, text="Exit", compound="left", command=exit_app,
          bg="#424242", fg="white", font=("Arial", 10, "bold")).grid(row=btn_row + 3, column=0, padx=10, pady=8)

# Output Summary Frames
display = tk.Label(reg_scrollable_frame, text="Queue: ", bg="#F5F5F5", fg="#212121",
                   font=("Arial", 10), justify="left", anchor="w")
display.grid(row=btn_row + 4, column=0, columnspan=2, pady=10, sticky="w")

processed_label = tk.Label(reg_scrollable_frame, text="Processed: ", bg="#F5F5F5", fg="#212121",
                           font=("Arial", 10), justify="left", anchor="w")
processed_label.grid(row=btn_row + 5, column=0, columnspan=2, pady=10, sticky="w")

# Purpose of Visit Aggregator Tab
purpose_frame = tk.Frame(notebook, bg="#FAFAFA")
notebook.add(purpose_frame, text="Purpose of Visit")

tk.Label(purpose_frame, text="All Patients Purpose Summary", font=("Arial", 14, "bold"), bg="#FAFAFA").pack(pady=10)
summary_text = tk.Text(purpose_frame, height=20, width=90)
summary_text.pack(pady=10, padx=20)


def update_purpose_summary():
    summary_text.delete("1.0", tk.END)
    text = "=== PURPOSE OF VISIT SUMMARY ===\n\n"
    for p in backend.queue + backend.processed_patients[-10:]:
        text += f"📋 {p['Full Name']} ({p['Ticket']}) - {p.get('Appointment Type', 'N/A')}\n"
        text += f"   → {p.get('Purpose of Visit', 'No notes')[:150]}...\n\n"
    summary_text.insert("1.0", text)


def on_tab_change(event):
    if notebook.tab(notebook.select(), "text") == "Purpose of Visit":
        update_purpose_summary()


notebook.bind("<<NotebookTabChanged>>", on_tab_change)

# Quiet database mapping load on initial initialization
load_data_bridge(silent=True)
